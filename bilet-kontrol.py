...
def driver_get(chrome_options):
    driver = webdriver.Chrome(options=chrome_options)
    driver.get("https://ebilet.tcddtasimacilik.gov.tr/view/eybis/tnmGenel/tcddWebContent.jsf")
    WebDriverWait(driver, 10).until(
        EC.visibility_of_element_located((By.CSS_SELECTOR, "#biletAramaForm > div:nth-child(3) > p:nth-child(4)")))
...
def sayfa_kontrol(driver):
    try:
        elem = WebDriverWait(driver, 10).until(EC.visibility_of_element_located(
            (By.XPATH, "//*[@id='mainTabView:gidisSeferTablosu:1:j_idt108:0:somVagonTipiGidis1_label']")))
        if elem != "":
...
if __name__ == "__main__":
    while True:
        print('Basladi')
        options = driver_setting()
        driver = driver_get(options)

        print('Kontrol ediliyor...')
        sayfa_kontrol(driver)
